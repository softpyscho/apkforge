...
def _load_build_cache() -> tuple[dict[str, str], dict[str, str]]:
    """Load versions and sources info cache generated by the build process."""
...
def _apk_sources_label(entry, sources_cache: dict[str, str]) -> str:
    """Generate markdown link for the actual APK download source used for this app."""
    labels = {
        "apkmirror": "APKMirror",
        "uptodown": "Uptodown",
        "apkpure": "APKPure",
    ...
    source_used = sources_cache.get(entry.table)
    
    if source_used and source_used in entry.dl_urls:
        url = entry.dl_urls[source_used]
    ...
    if entry.dl_urls:
        ...
        url = entry.dl_urls[first_src]
        name = labels.get(first_src, first_src.title())
        return f"[{name}]({url})"
