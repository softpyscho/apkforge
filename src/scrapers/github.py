...
class GitHubScraper(BaseScraper):
    ...
    def download(self, url: str, version: str, dest: Path, arch: str, dpi: str) -> DownloadResult:
        ...
        for a in apk_assets:
            name = a["name"]
            if version_f and version_f not in ("nightly", "latest", "auto") and version_f not in name:
                continue

            m = _ARCH_SUFFIX.search(name)
            file_arch = m.group(1).lower() if m and m.group(1) else "all"
            if arch in ("all", "both"):
                if file_arch != "all" and len(apk_assets) > 1:
                    continue
            else:
        ...
        if asset is None and apk_assets:
            asset = apk_assets[0]
        ...
        headers = self.net.gh_headers | {"Accept": "application/octet-stream"}
        try:
            self.net.download(asset["browser_download_url"], out_path, headers=headers)
        except NetworkError:
            self.net.download(asset["browser_download_url"], out_path, headers={"Accept": "application/octet-stream"})

        return DownloadResult(path=out_path, is_bundle=is_bundle, original_name=asset["name"])